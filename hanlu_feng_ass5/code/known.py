...
# read data from file
def read_supviesed_date_from_file(name):
        fp =open(name);
        dataX =[]
        dataY=[]
...
def supvised_method_k_mean(filename):
    data, dimension= read_supviesed_date_from_file(filename)
    cluster = iniCluster(data)# first step initilize the cluster the number is cluster length
    bradNclistInfolist= initlaClusterInfolist(cluster)# a brand new clusterInfolist on ly the number equeal the clusters' numer
    listclistInfo = getlabel(data,cluster,bradNclistInfolist)# get cluster list with its data and cluster
    cluster=[]
...
def get_origina_datar(filename):
        fp =open(filename);
        clusterlist=[]
        dataX =[]
        dataY=[]
        for line in fp.readlines():
            ...
            if(len(clusterlist)>0):
                for cluster in clusterlist:
                    if (cluster.getKnownId()==row[0]):# clusteriInfo have this cluster
                        n_row=[]
                        n_row.append(row[1])
                        n_row.append(row[2])
                        cluster.addatainfoList(n_row)
                        exist =1

                if (exist ==0):
                    clusterinfo = clusterInfo()
                    clusterinfo.setKnownID(row[0])
                    dateInfolist = []
                    n_row=[]
                    n_row.append(row[1])
                    n_row.append(row[2])
                    dateInfolist.append(n_row)
                    clusterinfo.setdatainfolist(dateInfolist)
...
def check_acccuracy(m, original,length):
    number=0

    for mm in m:# calculated cluster
           mm_datalist = mm.getdatainfoList()# data in calculated cluster
           bigfit =0
           tmp =0
           for org in original:# original cluster
               org_datalist = org.getdatainfoList()# data in original cluster

               for mmm in mm_datalist:
                   for orgorg in org_datalist:
                       if (mmm[0]==orgorg[0] and mmm[1]==orgorg[1]):
                           tmp = tmp+1

               if tmp >bigfit:
                   bigfit = tmp
               tmp =0
           number = number+bigfit
    value = float(number)/length
    print value



m,orgindata = supvised_method_k_mean("knowdata.csv")
org = get_origina_datar("knowdata.csv")
print "k-means accuracy:"
check_acccuracy(m,org, len(orgindata))
...
m, data = cal_knowlabel_em("knowdata.csv")
